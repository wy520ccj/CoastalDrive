...
0.8.1 独立版的旧证据只适用于当时合成的三段 WAV；它不包含后续替换的声音。玩家试听指出旧路噪过吵且不真实、发动机听感差，6B-01 的速度差碰撞分型也被实际试听判定不合格。0.8.3 已由 [Vehicle Impact Audio System v1](vehicle-impact-audio.md) 取代旧分型，新的实际听感仍需用户驾驶确认。
