...
- 6B-07 的远景护栏细边缘残余仍按原包 blocked，等待 Astra 判定；它不是 6B-08 的新问题。

## 本轮 6B-07

- 基线 `6d73d07cd4a1fca42b658f8f4e65a8fdf8e5eca7`，开工工作区干净。[任务包](tasks/6B-07-start-grid-rail-flicker.md)记录根因、修改和前后连续帧：起跑格同高标线争深度已局部修复；护栏阴影采样跳动明显减轻，但远景细顶边仍有像素级残余，**不宣称完全消除**。
- 滨海与无限高速真实离屏驾驶各按 720p/1080p、C 相机 11/16 m 录制，7 类情形共 28 对、1008 帧；代表性左右对照录像位于 `logs/6B-07/drive/`。T0 appearance 6项和 T1 appearance + core、3种子 headless 均通过，滨海/无限高速渲染 smoke 均通过。车轮阴影与场景阴影保留，未见标线悬浮或新增远景裁剪异常；可见窗口主观验收未做。
- 状态 blocked：Astra 需判定远景护栏亚像素边缘残余可否作为 6B-07 边界接受；若必须全消除，另立共享几何 LOD 或时间抗锯齿范围。本轮未动交通、道路物理、车辆、声音，也未开始 6B-08/09。
...
- 下一工作：先由玩家查看 6B-08 的真实截图/录像并在可见窗口操作确认界面主观质感；随后单独规划 6B-09 车辆材料和车库展示。6B-07 远景护栏残余仍由 Astra 单独判定。小地图、3 分钟正常驾驶录像与 1080p 帧耗时属于后续 6B Gate，不在 6B-08 内。当前主菜单显示中文“计时挑战”“滨海自由驾驶”“无限高速”；DRIVING 只是内部状态。音频诊断工具仅在将来出现具体退化时使用，见 [6B-05](tasks/6B-05-impact-calibration.md)。
