...
# 傍晚太阳相对车辆的方位：低角度、海面一侧偏前方，与天空着色器的太阳一致
SUN_OFFSET = Vec3(190, 210, 78)
...
@lru_cache(maxsize=1)
def cliff_texture():
    """岩壁：优先用下载的 Poly Haven CC0 贴图，缺失时回退到程序生成纹理。"""
    path = resource_root() / "assets/game/materials/cliff_side_diff_1k.jpg"
    if path.exists():
        texture = TexturePool.loadTexture(Filename.fromOsSpecific(str(path)))
        texture.setWrapU(Texture.WMRepeat)
        texture.setWrapV(Texture.WMRepeat)
...
class Scene:
    ...
    def setup_lighting(self) -> None:
        """傍晚光照：低角度暖阳 + 冷色补光 + 暖雾，天空与光照共用同一方向。"""
        coastal = self.base.session.simulation.track == "coastal"
        ambient = AmbientLight("coastal-ambient")
        ambient.setColor((0.26, 0.30, 0.40, 1) if coastal else (0.20, 0.24, 0.34, 1))
        ...
        # 暖白偏金，配合低角度形成长影；数值大于 1 用作强度
        sun_color = (1.55, 1.16, 0.78, 1) if coastal else (1.45, 1.10, 0.78, 1)
        ...
        # 地平线暖雾：远处山体与海面被夕阳染色，近景保持清晰
        haze = (0.86, 0.70, 0.55) if coastal else (0.80, 0.68, 0.58)
        self.base.setBackgroundColor(0.42, 0.60, 0.78)
        fog = Fog("coastal-haze")
        fog.setColor(*haze)
        fog.setExpDensity(0.00085 if coastal else 0.0013)
        self.render.setFog(fog)

    def update_lighting(self, position):
        # 太阳压低到海面上方，从右前方照射，与天空着色器里的太阳位置一致
        self.sun_path.setPos(position + SUN_OFFSET)
    ...
    def setup_scene(self) -> None:
        self.ocean = make_quad(
            "ocean", Vec3(0, 0, 0), 1800, 1800, Vec4(0.10, 0.58, 0.72, 1), SEA_LEVEL
        )
    ...
    def add_environment(self):
        for number, (prop, z) in enumerate(self.base.session.simulation.props):
            if prop.kind.startswith("checkpoint"):
                ...
                node.setH(prop.heading)
            node.reparentTo(self.render)
        self.add_inner_slope()

    def add_inner_slope(self):
        """内侧坡地的装饰带：近景山体 + 成排松林 + 坡脚岩块。

        山体是展示几何（同 `_add_ridge`），松林与岩块也只做视觉层次；
        全部放在远离行驶线的不可达区域，不参与物理。
        """
        rng = random.Random(20260924)
        length = map_length()
        for index in range(12):
            point = point_at(index * length / 12 + 8)
            position = offset_point(point, -112.0 - (index % 3) * 10.0)
            _add_ridge(self.render, make_mesh, position, 40 + (index % 3) * 6,
                       15 + (index * 7 % 14), index + 60)
        for index in range(60):
            point = point_at(index * length / 60)
            for row, offset in enumerate((-14.0, -22.0, -30.0, -38.0)):
                x, y, z = offset_point(point, offset + rng.uniform(-3.0, 3.0))
                filename = ("nature/tree_pineTallB.glb" if (index + row) % 3 == 0
                            else "nature/tree_pineTallA.glb")
                tree = self.load_model(filename)
                tree.setScale(3.4 + rng.uniform(0.0, 1.3) + row * 0.25)
                tree.setPos(x, y, z)
                tree.setH(rng.uniform(0.0, 360.0))
                tree.reparentTo(self.render)
        # 坡脚岩块贴着路基外侧的平地，避免半埋进坡体后露出破面
        for index in range(26):
            point = point_at(index * length / 26 + 5)
            x, y, z = offset_point(point, -19.0 - (index % 3) * 2.0)
            rock = self.load_model("nature/rock_largeA.glb")
            rock.setScale(1.7 + (index % 4) * 0.5)
            rock.setPos(x, y, z)
            rock.setH((index * 61) % 360)
